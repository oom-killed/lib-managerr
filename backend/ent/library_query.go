...
func (_q *LibraryQuery) QueryConnection() *ConnectionQuery {
...
func (_q *LibraryQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *LibraryQuery) FirstIDX(ctx context.Context) int {
...
func (_q *LibraryQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *LibraryQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *LibraryQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *LibraryQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *LibraryQuery) WithConnection(opts ...func(*ConnectionQuery)) *LibraryQuery {
	query := (&ConnectionClient{config: _q.config}).Query()
...
func (_q *LibraryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Library, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, library.ForeignKeys...)
	}
	...
	if query := _q.withConnection; query != nil {
		if err := _q.loadConnection(ctx, query, nodes, nil,
			func(n *Library, e *Connection) { n.Edges.Connection = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LibraryQuery) loadConnection(ctx context.Context, query *ConnectionQuery, nodes []*Library, init func(*Library), assign func(*Library, *Connection)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Library)
	for i := range nodes {
		if nodes[i].connection_libraries == nil {
			continue
		}
		fk := *nodes[i].connection_libraries
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "connection_libraries" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
